...
%define distname %{name}-%{version}_b4
...
# Fix for old distros that do not define %_datarootdir
%if ! %{?_datarootdir:1}0
...
BuildRequires: libgcc(%{__isa_name}-32)
%endif
%endif
...
%if %{?old_rpm}0
  echo
  echo "########################################################################"
  echo "Building 32bit libs on 64bit system not supported in this rpm version"
  echo "########################################################################"
  echo
%endif
...
MAKE[0]="./configure --with-components=modules --with-linux=\${kernelver} && make"
...
%changelog
* Tue Oct 21 2014 Olivier Lahaye <olivier.lahaye@cea.fr> 0.8.6-0.4
- 0.8.6-beta4
...
- Add support for old rpm versions (centos5) (missing %_datarootdir)
- disabled multilib support en old rpm versions:
  - defined variable not seen in %file sections
  - no support for arch specific deps (glibc-devel(x86-32))
* Wed Sep 18 2013 Olivier Lahaye <olivier.lahaye@cea.fr> 0.8.5-3
- DKMS package fixes.
* Thu Apr 11 2013 Olivier Lahaye <olivier.lahaye@cea.fr> 0.8.5-2
- Added support for DKMS.
* Tue Jan 29 2013 Paul H. Hargrove <PHHargrove@lbl.gov> 0.8.5-1
