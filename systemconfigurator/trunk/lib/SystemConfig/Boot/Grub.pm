package SystemConfig::Boot::Grub;
...
SystemConfig::Boot::Grub - Grub bootloader configuration module.
...
use SystemConfig::Boot::Label;
use SystemConfig::Boot::Devfs;
use Data::Dumper;
use SystemConfig::Util::Cmd qw(:all);
use SystemConfig::Util::Log qw(:print);
...
sub find_grub_root {
    my $this = shift;
    my $fstab = SystemConfig::Boot::Label::fstab_struct("/etc/fstab");
...
sub dev_transform {
    ...
    if(!$FSTAB) {
        $FSTAB = SystemConfig::Boot::Label::fstab_struct("/etc/fstab");
    }
