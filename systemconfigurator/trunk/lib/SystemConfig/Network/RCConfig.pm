package SystemConfig::Network::RCConfig;
...
SystemConfig::Network::RCConfig - rc.config Networking Module

=head1 SYNOPSIS

  my $networking = new SystemConfig::Network::RCConfig(%vars);
...
use Carp;
use vars qw($VERSION);
use base qw(SystemConfig::Network::Generic);
use SystemConfig::Util::FileMod;

$VERSION = sprintf("%d", q$Revision: 664 $ =~ /(\d+)/);

push @SystemConfig::Network::nettypes, qw(SystemConfig::Network::RCConfig);
...
sub setup_interfaces {
    ...
    my $rcfile = new SystemConfig::Util::FileMod(Seperator => '=', Quotes => '"');
    my $file = $this->chroot("/etc/rc.config");
...
L<SystemConifg::Network>, L<SystemConfig::Net::Netmask>, L<SystemConfig::Util::FileMod>, L<perl>
