package SystemConfig::Modules::SuSE8;
...
SystemConfig::Modules::SuSE8 - setup for /etc/rc.config for kernel modules

=head1 SYNOPSIS

 my $aliases = new SystemConfig::Modules::SuSE8(%vars);
...
use SystemConfig::Modules::Root;
use SystemConfig::Util::FileMod;
use vars qw($VERSION);

$VERSION = sprintf("%d", q$Revision: 664 $ =~ /(\d+)/);

push @SystemConfig::Hardware::conftypes, qw(SystemConfig::Modules::SuSE8);
...
sub new {
    ...
    my %this = (
                ...
                _filesmod => [],
                _configfile => "/etc/sysconfig/kernel",
               );
    bless \%this, $class;
}
...
sub footprint {
    my $this = shift;
    my $file = $this->chroot($this->{_configfile});
...
sub setup {
    ...
    # for the start get the root raid module(s)
  
    push @modules, get_root_raid_modules($this->chroot('/etc/fstab'));
    ...
    if(scalar(@modules)) {
        my $rc = new SystemConfig::Util::FileMod(Seperator => '=', Quotes => '"', Comment => '#');
        $rc->add_vars("INITRD_MODULES" => join(' ',@modules));
        $rc->update_file($this->chroot($this->{_configfile}));
