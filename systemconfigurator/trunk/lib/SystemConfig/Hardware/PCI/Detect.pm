package SystemConfig::Hardware::PCI::Detect;
...
use strict;
use Carp;
use SystemConfig::Hardware::PCI::Table qw(:all);

use vars qw($VERSION);

$VERSION = sprintf("%d", q$Revision: 664 $ =~ /(\d+)/);

push @SystemConfig::Hardware::hardtypes, qw(SystemConfig::Hardware::PCI::Detect);
...
SystemConfig::Hardware::PCI::Detect - detect pci devices from /proc/bus/pci/devices

=head1 SYNOPSIS

  my $detector = new SystemConfig::Hardware::PCI::Detect([root => $altroot]);
