...
BEGIN {
    ...
    @ARGV = qw(--confighw --cfgfile t/cfg/test.cfg);
}
...
open(TEST,">$dir/conf.modules");
close(TEST);

Hardware::setup($config);

ok(-e "$dir/conf.modules");

{
    local($/) = undef;
    open(IN,"<$dir/conf.modules") or croak("Can't open $dir/conf.modules");
    my $mcfile = <IN>;
...
# The following is a proc dump from my desktop.
